...
from rai_toolbox.mushin._perturbations import (
    _replace_best,
    random_restart,
    solve_perturbation,
)
...
l2p = partial(solve_perturbation, optimizer=L2ProjectedOptim)
...
def test_random_restart_repeats(Optim, x, use_best, repeats):
    if repeats < 1:
        with pytest.raises(ValueError):
            random_restart(solve_perturbation, repeats=repeats)
    else:
        perturber = random_restart(solve_perturbation, repeats=repeats)
        with torch.no_grad():
            model, x, y = pytorch_simple_model(x)
            loss = -F.cross_entropy(model(x), y, reduction="none")
...
def test_use_best(x, eps):
    ...
    # get best loss
    _, losses_best = solve_perturbation(
        model=model,
        data=x,
        target=y,
        optimizer=optimizer,
...
def test_steps_lr_equivalence_under_special_circumstances(
    ...
    adv_via_step, _ = solve_perturbation(
        model=IdentityModel(),
        data=data,
        target=target,
        optimizer=SGD,
    ...
    adv_via_lr, _ = solve_perturbation(
        model=IdentityModel(),
        data=data,
        target=target,
        optimizer=SGD,
