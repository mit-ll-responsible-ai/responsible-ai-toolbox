...
if PL_VERSION < Version(1, 6, 0):
    ...
    class HydraRerunDDP:  # type: ignore
        def __init__(self):
            raise ImportError(
                "`HydraRerunDDP` only supports PyTorch Lightning >= 1.6.0"
            )
    ...
    class HydraRerunDDP(DDPStrategy):
        ...
        def register_strategies(cls, strategy_registry: Dict) -> None:
            strategy_registry.register(  # type: ignore
            ...
            strategy_registry.register(  # type: ignore
                cls.strategy_name,
                cls,
                description=f"{cls.__class__.__name__}",  # type: ignore
            )
